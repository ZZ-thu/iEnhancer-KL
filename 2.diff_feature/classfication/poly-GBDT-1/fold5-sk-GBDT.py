...
dataFile = r'C:\workspace\PycharmProjects\poly-GBDT-1\data_lasso.mat'
data = scio.loadmat(dataFile)
X=data['data_lasso']

labelFile=r'C:\workspace\PycharmProjects\poly-GBDT-1\label.mat'
label=scio.loadmat(labelFile)
y=label['label']

rankFile = r'C:\workspace\PycharmProjects\poly-GBDT-1\indices.mat'
...
for k in range(1,6):
    ...
    # training model
    gb = GradientBoostingClassifier(learning_rate=0.2, n_estimators=370,
                max_depth=11,min_samples_split =500,min_samples_leaf =50, subsample=0.6)
    
    gb.fit(X_train,y_train)
    ans = gb.predict(X_test)
    joblib.dump(gb,'GBDT-data_lasso.model')
